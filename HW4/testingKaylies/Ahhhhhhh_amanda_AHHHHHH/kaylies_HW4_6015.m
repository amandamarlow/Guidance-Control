...
delta_t = 0.1; % KAYLIE COME BACK AND CHANGE
threshold = .0000001; % Kaylie come back and change
u_vec = [B; A; tf]; % initalizing control vector
iterations = 0;
options = odeset('RelTol',1e-12,'AbsTol',1e-12);
 weighted_norm_E0 = 100;
while (u_vec(7) > 1) % change to 0 or 5 or whatever.
    ...
    while (weighted_norm_E0 > threshold && (abs(iterations_comp_100(1)-iterations_comp_100(2)) > 1)) % add in dynamics for orr it stops changing kaylie come back
        ...
        if (iterations == 1 || mod(iterations,3) == 0) % KAYLIE COME BACK AND MOD WITH 100
        ...
        u_vec = u_vec_new;
        fprintf('Weighted norm = %f and it = %f and time final = %f \n',weighted_norm_E0, iterations, u_vec(7))
        %fprintf('      U (lambda) vec is: B = %f, %f, %f \n', u_vec(1), u_vec(2), u_vec(3))
        %fprintf('                         A is %f, %f, %f \n', u_vec(4), u_vec(5), u_vec(6))
        lala = 4;
